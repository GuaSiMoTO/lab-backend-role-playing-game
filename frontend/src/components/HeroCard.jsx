    ...
    <div className={`hero-scroll ${selectedClass}`} id={`card-${hero.id}`}>
      ...
      <div className="scroll-body">
        ...
        >
          <div style={{ flex: 1 }}>
            ...
            <div className="scroll-sub">
              <span>
                {SPECIES_ICON[hero.especie] || "👤"} {hero.especie}
              </span>
              <span className="sep">·</span>
              <span>{hero.categoria}</span>
            </div>
          </div>
          <div className={`wax-seal seal-${hero.categoria}`}>
            {CAT_LABEL[hero.categoria]}
          </div>
        </div>
        ...
        <div className="scroll-record">
          Victorias: <span>{hero.victorias}</span> &nbsp;·&nbsp; Derrotas:{" "}
          <span>{hero.derrotas}</span>
        </div>
      </div>
      <div className="scroll-actions">
        <button
          className="ink-btn btn-red"
          onClick={() => onPickSlot1(hero.id)}
        >
          ① Campeón
        </button>
        <button
          className="ink-btn btn-blue"
          onClick={() => onPickSlot2(hero.id)}
        >
          ② Rival
        </button>
        <button className="ink-btn btn-del" onClick={() => onDelete(hero.id)}>
          Tachar
        </button>
      </div>
    </div>
